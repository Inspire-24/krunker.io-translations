module.exports = `
menu.btn.ranked रैंक का खेल
menu.btn.shop दुकान
menu.btn.social हब
menu.btn.maps मैप्स
menu.btn.mods मॉड
menu.btn.settings सेटिंग
menu.btn.customize रूप बदले
menu.btn.invite इंवाइट करें
menu.btn.invite.clicked कॉपी
menu.btn.join जॉइन
menu.btn.host होस्ट करें
menu.btn.browser खेल ढूंढें
generic.wait कृपया प्रतीक्षा करें...
generic.loading लोड हो रहा है । ..
generic.none कुछ भी नहीं
generic.connecting कनेक्टिंग...
generic.select चुनें
generic.delete हटाएं
generic.save सेव करें
generic.load लोड
generic.search खोजें
generic.leave छोड़ें
generic.default डिफ़ॉल्ट
generic.submit सबमिट करें
generic.create बनाएँ
generic.random रैंडम
generic.failed असफल
generic.success सफलता
generic.name नाम
generic.level लेवल
generic.score स्कोर
generic.off बंद
generic.defeat हार
generic.victory विजय
generic.matchover मैच ख़तम
generic.matchaband मैच छोड़ दिया
generic.follow फॉलो
generic.unfollow अनफॉलो
generic.followers फॉलोवर्स
generic.following फॉलोइंग
generic.games खेल
generic.wins जीत
generic.lose हार
generic.clan क्लैन
generic.games.played खेले गये खेल
generic.games.won खेल जीते
generic.games.lost खेल हारे
generic.timep कितना समय खेला
generic.kills हत्या
generic.deaths मौतें
generic.kdr KDR
generic.time समय
generic.logout लॉगआउट
generic.logout.sessions सभी सत्रों से लॉगआउट
generic.username यूजर का नाम
generic.password पासवर्ड
generic.register रजिस्टर
generic.login लॉगिन
generic.accuracy सटीकता
player.waiting खिलाड़ियों का इंतजार कर रहे हैं ...
player.died तुम मर गए
player.killed द्वारा मारे गए
player.killstreak {0} on a {1} Kill Streak
player.inactive इनैक्टिव
player.disconnect डिस्कनेक्ट
timer.end अगला दौर {0} में
leaderboard.empty खाली लॉबी
popup.checkpoint चैकपौइंट
app.play खेलने के लिए क्लिक करें
app.spectate स्पैक्टेट करे
app.play.mobile खेलने के लिए टैप करें
app.spectate.mobile स्पैक्टेट करे
app.play.controller START दबाएं
app.spectate.controller स्पैक्टेट करे
purchase.error ERROR
purchase.error.click यहां क्लिक करें  </a> और फिर से प्रयास करें!
purchase.error.limit आप अपनी दैनिक लेनदेन सीमा को पार कर चुकें हैं
settings.reset रीसेट
settings.reset.confirm क्या आप वाकई अपनी सभी सेटिंग्स रीसेट करना चाहते हैं?
settings.import इम्पोर्ट
settings.export ऐक्सपोर्ट
settings.none कोई सेटिंग नहीं मिली
settings.backup बैकअप सेटिंग्स
settings.controls.change Keybinds
settings.controls.press किसी भी Key को दबाएं
settings.local.header Localization <span id="requiresRestart"><span style="color: #eb5656">*</span> requires restart</span>
settings.local.region डिफ़ॉल्ट क्षेत्र
settings.local.lang भाषा
settings.network.header नेटवर्क
settings.network.sendRate ज़्यादा Tickrate
settings.network.netRerouting ऑटो राउटिंग (पिंग को प्रभावित करता है)
settings.network.lagComp लैग काम्पन्सेशन
settings.popup.header पॉपअप
settings.popup.freeKRPop मुफ्त KR पॉपअप
settings.experimental.header आजमाइशी
settings.experimental.aimFreezeFix लक्ष्य के दौरान फ्रीज को ठीक करें
settings.experimental.mouseFlickFix लक्ष्य के दौरान झटके को ठीक करें
settings.experimental.shaderRendering शेडर रैडंर
settings.client.header Client क्लाइंट
settings.client.unlimitedFrames असीमित FPS
settings.client.gameCap खेल कैप्चर
settings.client.experimental आजमाइशी फ़ीचर
settings.client.discordRPC डिस्कॉर्ड आर पी सी
settings.client.resourceSwapper साधन अदला-बदली
settings.client.clearCache Cache साफ़
settings.client.resetSettings रीसेट क्लाइंट सेटिंग
settings.client.acceleratedCanvas एक्सेलरेटेड कैन्वस
settings.client.angleBackend ऐंगल ग्राफ़िक्स बैकेंड
settings.client.colorProfile रंग प्रोफ़ाइल
settings.dev.header डेवलपर
settings.dev.forceUpdate बलपूर्वक श्रृंगार बदलें
settings.dev.forceRegen हमेशा खिलाड़ी को पुनर्जीवित करें
settings.dev.extraBan बैन की अतिरिक्त जानकारी
settings.chat.header चैट
settings.chat.chat चैट बॉक्स दिखाएँ
settings.chat.chat.all हमेशा
settings.chat.chat.focus फोकस करने पर
settings.chat.messages खिलाडियों के सन्देश दिखाएं
settings.chat.unboxings अनबॉक्सिंग दिखाएं
settings.chat.chatOp चैट अपारदर्शिता
settings.chat.chatBGOp चैट BG अपारदर्शिता
settings.chat.chatHeight चैट ऊंचाई
settings.chat.chatTimer चैट टाइमर (s)
settings.chat.chatTextOutline चैट टेक्स्ट का बॉर्डर
settings.chat.chatFilter अपशब्दों को हटाएँ
settings.quality.header पर्फोर्मेन्स
settings.quality.updRate फ़्रेम कैप
settings.quality.rendDst रेंडर दूरी
settings.quality.res रेसोलुशन
settings.quality.part कण
settings.quality.mapDet नक्शा विवरण
settings.quality.noTex कोई टेक्सचर नहीं
settings.quality.partdist कण दूरी
settings.quality.renderdist रेंडर दूरी
settings.quality.reflection प्रतिबिंब की गुणवत्ता
settings.quality.antiAlias एंटीअलियासिंग
settings.quality.shadows छाया
settings.quality.softShad नरम छाया
settings.quality.highResShad उच्च रेसोलुशन छाया
settings.quality.shadowsDynamic डायनामिक छाया
settings.quality.trails बुलेट ट्रेल्स
settings.quality.yourTrails अपने ट्रेल्स
settings.quality.mflash मज़ल फ्लैश
settings.quality.flap स्नाइपर फ्लैप
settings.quality.lowSpec कम स्पेसिफ़िकेशन
settings.quality.ambient एम्बियेन्ट शेडींग
settings.quality.ambientOld पुरानी शेडींग (जब संभव हो)
settings.quality.textureAnim टेक्सचर एनीमेशन
settings.quality.objectAnim वस्तु एनिमेशन
settings.quality.screenShake स्क्रीन शेक
settings.quality.weaponShine हथियार की चमक
settings.quality.rendGamma गामा
settings.quality.lighting प्रकाशन
settings.quality.showExplo विस्फोट दिखाएँ
settings.quality.aspectRatio आस्पेक्ट रेशियो
settings.post.header पोस्ट प्रोसेसिंग
settings.post.post पोस्ट प्रोसेसिंग
settings.post.bloom ब्लूम
settings.post.bloomThreshold ब्लूम थ्रेशोल्ड
settings.post.bloomStrength ब्लूम ताकत
settings.post.bloomRadius ब्लूम रेडियस
settings.post.ssao SSAO
settings.post.ssaoRadius SSAO रेडियस
settings.interface.hitInd निशान लगने पर संकेत दिखाएँ
settings.interface.hitColor निशान संकेतों का रंग
settings.interface.dmg निशान लगने पर डैमेज दिखाएँ
settings.interface.dmgCol डैमेज संकेतों का रंग
settings.interface.critCol घातक डैमेज संकेतों का रंग
settings.interface.header इंटरफ़ेस
settings.interface.scale UI का माप
settings.interface.ui UI दिखाएँ
settings.interface.scoreboard पुराने स्कोरबोर्ड का उपयोग करें
settings.interface.oldBrowser पुराने ब्राउज़र का उपयोग करें
settings.interface.rarityanim दुर्लभता एनिमेशन असमर्थ करें
settings.interface.showPfp प्रोफ़ाइल चित्र दिखाएँ
settings.interface.showTradeMsg व्यापार के संदेश दिखाएँ
settings.interface.hp HP
settings.interface.dmgScale डैमेज संकेतों का माप
settings.interface.killf Kill Feed दिखाएँ
settings.interface.ping पिंग दिखाएँ
settings.interface.net-stats नेटवर्क आँकड़े दिखाएँ
settings.interface.fps FPS दिखाएँ
settings.interface.deaths मौत के आंकड़े दिखाएँ
settings.interface.kd K/D दिखाएँ
settings.interface.kills हत्या के आंकड़े दिखाएँ
settings.interface.streak स्ट्रीक काउंटर दिखाएँ
settings.interface.score स्कोर काउंटर दिखाएँ
settings.interface.spectators दर्शक काउंटर दिखाएँ
settings.interface.speed गति काँटा दिखाएं
settings.interface.speedOffX गति काँटे का X ऑफसेट
settings.interface.speedOffY गति काँटे का Y ऑफसेट
settings.interface.speedOpacity गति काँटे की पारदर्शकता
settings.interface.speedScale स्पीड स्केल
settings.interface.speedColor गति काँटे का रंग (वर्तमान)
settings.interface.speedColorMax गति काँटे का रंग (अधिकतम)
settings.interface.medals पदक दिखाएं
settings.interface.hideNames खिलाड़ी के नाम दिखाएँ
settings.interface.hideNames.team केवल अपनी टीम के
settings.interface.hideNames.enemy केवल दुश्मन के
settings.interface.hideNames.all सभी के
settings.interface.nametagStyle नैम-टैग स्टाइल
settings.interface.nametagOpac नैम-टैग की पारदर्शकता
settings.interface.nametagStyle.health केवल HP
settings.interface.nametagStyle.name केवल नाम
settings.interface.nametagStyle.nameLvl केवल नाम और लेवल
settings.interface.nametagStyle.all सब कुछ
settings.interface.healthNum नैम-टैग HP संख्या
settings.interface.healthColT नैम-टैग अपनी टीम के HP का रंग
settings.interface.healthColE नैम-टैग दुश्मन के HP का रंग
settings.interface.xpBarCol XP बार का रंग
settings.interface.xpBarOpac XP बार की पारदर्शकता
settings.crosshair.header Crosshair
settings.crosshair.type Crosshair का ढंग
settings.crosshair.type.2 कस्टम
settings.crosshair.type.3 परतों वाला
settings.crosshair.type.4 चित्र
settings.crosshair.type.5 प्रिसिजन
settings.crosshair.style शैली
settings.crosshair.style.0 क्रॉस
settings.crosshair.style.1 खोखला सर्कल
settings.crosshair.style.2 ठोस सर्कल
settings.crosshair.style.3 खोखला चौकोर
settings.crosshair.style.4 ठोस चौकोर
settings.crosshair.image छवि
settings.crosshair.image.paste Crosshair चित्र
settings.crosshair.always हमेशा दिखाओ
settings.crosshair.color रंग
settings.crosshair.shadow.color छाया रंग
settings.crosshair.shadow.thickness छाया मोटाई
settings.crosshair.size नाप
settings.crosshair.thickness मोटाई
settings.crosshair.gap गैप
settings.crosshair.dot बिंदु
settings.crosshair.opacity.use कस्टम अपारदर्शिता का उपयोग करें
settings.crosshair.opacity.custom कस्टम अपारदर्शिता
settings.hitmarker.header हिटमार्कर
settings.hitmarker.color रंग
settings.hitmarker.kcolor हत्या करने पर रंग
settings.hitmarker.thickness मोटापा
settings.hitmarker.opacity अपारदर्शिता
settings.hitmarker.leng लंबाई
settings.hitmarker.spacing अंतर
settings.hitmarker.animD एनीमेशन का आकार
settings.hitmarker.animS एनिमेशन की गति
settings.hitmarker.hitm दिखाओ
settings.hitmarker.hitmFad फेड गति
settings.hitmarker.custom हिटमार्कर इमेज
settings.hitmarker.custom.paste हिटमार्केर इमेज URL
settings.onKillS.style.0 डिफ़ॉल्ट
settings.onKillS.style.1 रेसोल्युशन कम करें
settings.onKillS.style.2 FOV कम करें
settings.onKillS.style.3 FOV बढ़ाए
settings.serverb.header सर्वर खोजे
settings.serverb.oldBrowser पुराना ब्राउज़र
settings.serverb.defaultRegionOnly केवल डिफ़ॉल्ट क्षेत्र
settings.gameprivacy.header खेल की गोपनीयता
settings.gameprivacy.streamer स्ट्रीमर मोड
settings.gameprivacy.anon गुमनाम मोड
settings.gameprivacy.hideGameTitle खेल शीर्षक छिपाएं
settings.gameplay.header गेमप्लेर
settings.gameplay.sensitivityX X-संवेदनशीलता
settings.gameplay.sensitivityY Y-संवेदनशीलता
settings.gameplay.aimsensX लक्ष्य X-संवेदनशीलता
settings.gameplay.aimsensY लक्ष्य Y-संवेदनशीलता
settings.gameplay.challenge चैलेंज मोड
settings.gameplay.invert इन्वर्ट Y-एक्सिस
settings.gameplay.onkillS मारने पर
settings.gameplay.scrollDir स्क्रॉल दिशा
settings.controller.header कन्ट्रोलर
settings.controller.sensitivityX X-संवेदनशीलता
settings.controller.disableCons कंट्रोलर्स बंद करें
settings.controller.sensitivityY Y-संवेदनशीलता
settings.controller.aimsensX लक्ष्य X-संवेदनशीलता
settings.controller.aimsensY लक्ष्य Y-संवेदनशीलता
settings.controller.deadZoneL डेड जोन (बाएं)
settings.controller.deadZoneR डेड जोन (दाएं)
settings.controller.triggerThres ट्रिगर थ्रेसहोल्ड
settings.controller.vibration कंपन
settings.controller.gradSpeed ग्रॅजुअल स्पीड
settings.controller.invert इन्वर्ट Y-एक्सिस
settings.mobile.header मोबाइल
settings.mobile.sensitivityX X- संवेदनशीलता
settings.mobile.sensitivityY Y- संवेदनशीलता
settings.mobile.aimsensX लक्ष्य X-संवेदनशीलता
settings.mobile.aimsensY लक्ष्य Y-संवेदनशीलता
settings.mobile.vibration कंपन
settings.mobile.gradSpeed ग्रॅजुअल स्पीड
settings.mobile.invert इन्वर्ट Y-एक्सिस
settings.audio.header आवाज
settings.audio.sound मास्टर वॉल्यूम
settings.audio.ambient एम्बियेन्ट वॉल्यूम
settings.audio.voice वाणी वॉल्यूम
settings.audio.weps हथियार वॉल्यूम
settings.audio.player खिलाड़ी की वॉल्यूम
settings.audio.skin पेहनावे की वॉल्यूम
settings.audio.ui UI वॉल्यूम
settings.audio.asset एसेट वॉल्यूम
settings.audio.dialogue एक्शन वॉल्यूम
settings.viewmodel.header व्यू मॉडल
settings.viewmodel.fov FOV
settings.viewmodel.fps हथियार की FOV
settings.viewmodel.bobbing हथियार बोब्बिंग
settings.viewmodel.leaning हथियार लीनिंग
settings.viewmodel.show.primary चाकू दिखाओ
settings.viewmodel.show.secondary पिस्तौल दिखाओ
settings.viewmodel.roundArms गोल हाथ
settings.viewmodel.show.melee मिली दिखाओ
settings.viewmodel.show.ads ADS पर हथियार छुपाओ
settings.viewmodel.toggleAim टॉगल लक्ष्य
settings.viewmodel.show.hands हाथ दिखाओ
settings.viewmodel.leftHand बाएं हाथ से
settings.viewmodel.aimAnim हथियार लक्ष्य एनीमेशन
settings.viewmodel.rot हथियार रोटेशन
settings.viewmodel.xOff हथियार X-ऑफसेट
settings.viewmodel.yOff हथियार Y-ऑफसेट
settings.viewmodel.zOff हथियार Z-ऑफसेट
settings.viewmodel.weapSwapY हथियार स्वाप-Y
settings.viewmodel.weapReloadY हथियार रीलोड Y
settings.viewmodel.yOffADS हथियार ADS Y-ऑफसेट
settings.viewmodel.adsFovMlt ADS FOV की शक्ति
settings.editing.header एडिटिंग
settings.editing.health.high HUD उच्च हेल्थ
settings.editing.health.low HUD कम हेल्थ
settings.editing.speedLines स्पीड लाइंस
settings.editing.spdLinesCol स्पीड लाइंस रंग
settings.editing.score.color पॉपअप स्कोर रंग
settings.editing.score.show पॅापअप स्कोर दिखाओ
settings.editing.score.scale पॉपअप स्कोर स्केल
settings.editing.score.shadow पॉपअप स्कोर छाया
settings.editing.scoreOffX पॉपअप स्कोर X-ऑफसेट
settings.editing.scoreOffY पॉपअप स्कोर Y-ऑफसेट
settings.editing.prog.color प्रोग्रेस बार रंग
settings.editing.prog.opacity प्रोग्रेस बार अपारदर्शिता
settings.editing.prog.scale प्रोग्रेस बार स्केल
settings.editing.prog.shadow प्रोग्रेस बार छाया
settings.editing.progOffX प्रोग्रेस बार X-ऑफसेट
settings.editing.progOffY प्रोग्रेस बार Y-ऑफसेट
settings.editing.progRot प्रोग्रेस बार रोटेशन
settings.editing.progm.color चार्ज बार रंग
settings.editing.progm.opacity चार्ज बार अपारदर्शिता
settings.editing.progm.scale चार्ज बार नाप
settings.editing.progm.shadow चार्ज बार छाया
settings.editing.progmOffX चार्ज बार X ऑफसेट
settings.editing.progmOffY चार्ज बार Y ऑफसेट
settings.editing.progmRot चार्ज बार रोटेश
settings.editing.saturation सेचुरेशन
settings.editing.menuSaturation सेचुरेशन (UI)
settings.editing.hue रंग (खेल में)
settings.editing.menuHue रंग (UI)
settings.editing.vignette विग्नेट
settings.editing.feedLimit किलफीड सीमा
settings.editing.depth डेप्थ मैप
settings.editing.green ग्रीन स्क्रीन
settings.editing.tracercol बुलेट  ट्रेसर का रंग
settings.editing.tracer बुलेट  ट्रेसर
settings.editing.tracer.offset ट्रेसर ऑफसेट
settings.shaders.header शेडर इनपुट्स
settings.mods.header अनुकूलन
settings.mods.load लोड मॉड्स
settings.mods.logo लोगो परिवर्तन की अनुमति दें
settings.mods.auto ऑटो-लोड मॉड
settings.mods.borders स्कोप बॉर्डर्स
settings.mods.borderCol बॉर्डर्स रंग
settings.mods.borderOpac स्कोप बॉर्डर्स अपारदर्शिता
settings.mods.scope स्कोप इमेज
settings.mods.scope.paste स्कोप इमेज URL
settings.mods.scope.opacity स्कोप अपारदर्शिता
settings.mods.scope.width स्कोप इमेज चौड़ाई
settings.mods.scope.height स्कोप इमेज ऊंचाई
settings.mods.dot रैटीकल
settings.mods.dot.width रैटीकल इमेज चौड़ाई
settings.mods.dot.height रैटीकल इमेज ऊंचाई
settings.mods.dot.paste रैटीकल URL
settings.mods.endmessage मैच समाप्ति संदेश
settings.mods.profile प्रोफाइल इमेज
settings.mods.profile.paste प्रोफाइल इमेज URL
settings.mods.ammo अम्मॉ आइकॉन इमेज
settings.mods.ammo.paste अम्मॉ आइकॉन इमेज URL
settings.mods.kills किल्क आइकॉन इमेज
settings.mods.kills.paste हत्या आइकॉन इमेज URL
settings.mods.deaths मौतें आइकॉन इमेज
settings.mods.deaths.paste मौतें आइकॉन इमेज URL
settings.mods.streak स्ट्रीक काउंटर आइकॉन इमेज
settings.mods.streak.paste स्ट्रीक काउंटर आइकॉन इमेज URL
settings.mods.timer टाइमर आइकॉन इमेज
settings.mods.timer.paste टाइमर आइकॉन इमेज URL
settings.mods.usedamage डैमेज ओवरले का उपयोग करें
settings.mods.damage डैमेज ओवरले की इमेज
settings.mods.damage.paste डैमेज ओवरलेस इमेज URL
settings.mods.overlay खेल ओवरले इमेज
settings.mods.overlay.paste खेल ओवरले इमेज URL
settings.social.region रीजन स्टैट्स
windows.settings.header खेल समायोजन
windows.mail.header क्रंकर मेल
windows.servers.header सर्वर
windows.servers.none कोई जुड़ने योग्य खेल नहीं मिला...
windows.servers.host + होस्ट गेम
windows.servers.hide फ़ुल सर्वर छिपाए
windows.servers.priority रीजन की प्राथमिकता
windows.servers.online {0} खेल में
windows.servers.social {0} सोशल में
windows.loadout.header लोडआउट बदले
windows.loadout.class क्लास
windows.loadout.primary प्राइमरी
windows.loadout.primary.attach अटैचमेंट
windows.loadout.primary.reticle रैटीकल
windows.loadout.primary.scope स्कोप
windows.loadout.secondary सेकेंडरी
windows.loadout.secondary.skin सेकेंडरी स्किन
windows.loadout.melee मिली
windows.loadout.hat हैट आइटम
windows.loadout.back बैक आइटम
windows.loadout.waist कमर आइटम
windows.loadout.shoe जूते
windows.loadout.face चेहरा
windows.loadout.spray स्प्रे
windows.loadout.color.skin त्वचा का रंग
windows.loadout.color.chat चैट मे अपने नाम का रंग
windows.loadout.color.hair बाल का रंग
windows.loadout.dye डाई
windows.loadout.pet पेट
windows.loadout.stat किलकार्ड स्टैटक
windows.loadout.flag देश का झंडा
windows.mods.drop यहाँ मॉड फ़ाइल ड्रॉप या क्लिक करें
windows.mods.paste पेस्ट मॉड URL
windows.mods.load मॉड लोड करे
windows.mods.none कोई मॉड नहीं मिला
windows.mods.header मॉड मैनेजर
windows.mods.guide मॉड गाइड
windows.mods.list मॉड लिस्ट
windows.mods.load.header लोड मॉड
windows.mods.publish मॉड पब्लिश करे
windows.mods.viewer व्यूअर
windows.account.header अकाउंट
windows.account.recover अकाउंट वापस पाने के लिए  <a>recovery@yendis.ch</a> से संपर्क करें
windows.class.header क्लास चुनें
windows.controls.header कंट्रोल्स बदलें
windows.controls.forward आगे
windows.controls.backward पीछे
windows.controls.left बाएं
windows.controls.right दायें
windows.controls.reload रीलोड
windows.controls.aim एम्
windows.controls.shoot गोली चलाना
windows.controls.inspect इंस्पेक्ट
windows.controls.spray स्प्रे
windows.controls.jump कूदना
windows.controls.crouch झुकना
windows.controls.prim प्राइमरी हथियार
windows.controls.swap सेकेंडरी हथियार
windows.controls.melee मिली हथियार
windows.controls.equip सामान लैस करे
windows.controls.chat चैट
windows.controls.voice माइक्रफोन
windows.controls.list प्लेयर सूची
windows.controls.board लीडरबोर्ड
windows.controls.interact इंटरैक्ट
windows.controls.interactSec सेकेंडरी इंटरैक्ट
windows.controls.confirm कन्फर्म करे
windows.controls.drop ड्रॉप
windows.controls.wepVis हथियार की विजिबिलिटी टॉगल करें
windows.controls.streak0 स्ट्रीक 1
windows.controls.streak1 स्ट्रीक 2
windows.controls.streak2 स्ट्रीक 3
windows.controls.streak3 स्ट्रीक 4
windows.controls.streak4 स्ट्रीक 5
windows.controls.premium0 नोक्लिप
windows.controls.premium1 गाड मोड
windows.controls.premium2 खुद को नष्ट करे
windows.controls.premium3 स्माइट
windows.controls.reset स्थान रिसेट करे (रेस & पार्कोर)
windows.controls.pretend खेल छुपाए
windows.host.header कस्टम खेल होस्ट करे
windows.host.maps सर्वर सेटअप
windows.host.cmaps सामुदायिक खेल
windows.host.raw रॉ गेम डेटा
windows.host.modes मोड का चयन करें
windows.host.classes क्लासेज चुनें
windows.host.settings सर्वर सेटिंग
windows.host.preset होस्ट प्रीसेट
windows.host.preset.load प्रीसेट लोड हो रहा है...
windows.host.preset.name प्रीसेट नाम
windows.host.start खेल शुरू
windows.host.rehost खेल शुरू (खिलाड़ियों को लाये)
windows.spray.header स्प्रे का चयन करें
windows.spray.default नि: शुल्क स्प्रे
windows.spray.spin स्प्रे
windows.maps.header कस्टम्स खेल
windows.maps.none कोई खेल नहीं मिला!
windows.maps.name खेल का नाम
windows.maps.editor खेल रचनाकार
windows.maps.publish खेल को प्रकाशित करें
windows.maps.community रचनाकार समुदाय
windows.publish.map.header खेल को प्रकाशित करें या खेल अप्डेट करें
windows.publish.map.thumbnail थंबनेल नहीं है
windows.publish.map.paste खेल डेटा पेस्ट करे
windows.publish.map.update प्रकाशित करें /अप्डेट करें
windows.publish.map.login खेल बनाने और अपलोड करने के लिए!
windows.theatre.header थिएटर मोड
windows.theatre.record रिकॉर्डिंग करना सक्षम करें
windows.theatre.key रिकॉर्ड की
windows.clans.header क्लान
windows.clans.members सदस्यों ({0})
windows.clans.requests अनुरोध सूची
windows.clans.requests.none कोई अनुरोध नहीं है
windows.clans.page [{0}] क्लान पेज
windows.clans.create क्लान बनाए
windows.clans.name क्लान का नाम
windows.clans.view क्लान दिखाए
windows.clans.join क्लान में जुड़िये
windows.clans.login क्लान बनाने और जुड़ने के लिए!
windows.store.header बाज़ार
windows.store.spin {0} स्पिन
windows.store.agree इस खरीद के लिए, आप {0} शर्तों से सहमत हैं।
windows.store.purchase KR खरीदे
windows.store.voucher वाऊचर पाये
windows.store.voucherinfo अपने पसन्दीदा स्ट्रीमर या कार्यक्रम से वाऊचर पाये
windows.store.market बाजार
windows.store.market2 क्रंकर बाजार
windows.store.premium प्रीमियम खरीदे
windows.store.premium2 क्रंकर प्रीमियम
windows.store.buyP अपना पसंदीदा नाम और अन्य लाभ के लिए, खरीदे
windows.store.buy वस्तुए खरीदे और बेचे
windows.store.wheels प्राइज व्हील्स
windows.store.unlock नए हथियार स्किन्स और वस्तुए को अनलॉक कीजिए
windows.store.amount अकाउंट में {0} KR है
windows.store.login बाजार से आइटम्स अनलॉक करने के लिए!
windows.store.purchasing खरिदा जा रह है...
windows.store.unboxed {0}{1} अनबॉक्स्ड
windows.trade.header व्यापार करना
windows.scope.header स्कोप चुने
windows.premium.header प्रीमियम
windows.skin.header स्किन चुने
windows.hat.header टोपी चुने
windows.back.header बैक चुने
windows.waist.header कमर की वस्तु चुने
windows.face.header चेहरा चुने
windows.melee.header मिली चुने
windows.attach.header अटैचमेंट चुने
windows.reticle.header रेटिकल चुने
windows.reticle.unlocked {0} पर अनलॉक हो जाता है
windows.reticle.req {0} पर अनलॉक हो जाता है
windows.dye.header डाई चुने
windows.dye.default डिफ़ॉल्ट डाई
windows.dye.spin डाई
windows.changelog.header चंगेलोग
windows.mods.community.header सामुदायिक मोड्स
windows.mods.community.fav आपका पसंदीदा
windows.mods.community.new नए मोड्स
windows.mods.community.hot हॉट
windows.mods.community.name मॉड का नाम
windows.mods.community.none मॉड नहीं मिला
windows.publish.mod.header मॉड को प्रकाशित कीजिए
windows.publish.mod.thumbnail नो थंबनेल
windows.publish.mod.name मॉड का नाम
windows.publish.mod.paste मॉड Url
windows.publish.mod.update प्रकाशित/अपडेट
windows.publish.mod.login मॉड बनाने और अपलोड करने के लिए!
windows.secondary.header सेकेंडरी चुने
windows.secondary.unlocked उनलॉक्ड
windows.secondary.req {0} पर अनलॉक हो जाता है
windows.advertise.header विज्ञापित
windows.support.header सपोर्ट
windows.players.header प्लेयर लिस्ट
windows.players.none कोई प्लेयर्स नहीं...
windows.join.header जोइन
windows.join.code एंटर खेल URL/Code
windows.client.header क्लाइंट
windows.client.windows Windows इंस्टालर
windows.client.mac macOS इंस्टालर
windows.client.linux Linux इंस्टालर
windows.ranked.header रैन्क्ड
windows.ranked.login रैन्क्ड खेल खेलने के लिए!
windows.ranked.level आपको कम से कम  <span color="black">लेवल  {0}</span> का होना जरूरी है रंकड खेल खेलने के लिए! आपका अभी यह  <span color="black">level {1}</span>.
windows.ranked.mode.r1v1 सोलो
windows.ranked.mode.r2v2 डुओ
windows.ranked.mode.r4v4 क्वैड
windows.chall.header चुनौती
windows.chall.login चुनौतियों को ख़तम  करने के लिए
thumbnail.upload.limit Failed. {0}kb थंबनेल  सीमा
image.upload.limit Failed. {0}kb इमेज  सीमा
thumbnail.upload.error थंबनेल एरर
mod.extracting extracting {0}/{1}
mod.disabled मोड्स  डिसेबल्ड
mod.error मोड़ एरर!
mod.loading मॉड लोड हो रहा हे ...
mod.invalid इनवैलिड मोड़ फाइल !
matchmaker.full फूल
matchmaker.full2 खेल भरा हुआ है .
matchmaker.updating खेल अपडेट हो रहा हे
matchmaker.invalid खेल नहीं मिला.
matchmaker.none सारे सरवर फूल हे
adblocker KR प्राप्त करने के लिए, अपना adblocker बंद करें
ad-failed विज्ञापन लोड होने में असफल हुआ । कृपया पुन: प्रयास करें
streamers.views {0} दर्शक
streamers.none कोई सक्रिय स्ट्रीम्स नहीं!
error.extentions यदि आपके पास कोई एक्सटेंशन है, तो उन्हें बंद करें
error.seek क्लिक करने से नए खेल ढूंढ़ने की कोशिश करे
custom.map.upload.limit फेल्ड. 5,00,000 अक्षर गेम डाटा लिमिट रीच्ड
custom.map.missing कम से कम एक गेम या मैप चुनें.
custom.mode.missing कम से कम एक  mode चुनें
custom.class.missing कम से कम एक क्लास  चुनें
server.config.maxPlayers प्लेयर्स
server.config.minPlayers न्यूनतम प्लेयर्स
server.config.lives जिन्दगिया
server.config.gameTime मिनट्स
server.config.gravMlt ग्रेविटी
server.config.tickRI ज्यादा टिक रेट
server.config.maxPS 40 खिलाड़ियों का सर्वर
server.config.promServ सर्वर प्रमोट करे
server.config.jumpMlt कूदने की शक्ति
server.config.airStrf हवा मे स्ट्रेफिंग
server.config.deltaMlt खेल की गति
server.config.strafeSpd स्ट्रेफ गति
server.config.fiRat फायर रेट
server.config.reSpd रीलोड करने की गति
server.config.slidTime स्लाइड करने का समय
server.config.fixMov फिक्स्ड मूवमेंट
server.config.slidSpd स्लाइड करने की गति
server.config.healthMlt हेल्थ मल्टिप्लायर
server.config.impulseMlt हथियार इंपल्स
server.config.nameTeam1 दल 1 नाम
server.config.nameTeam2 दल 2 नाम
server.config.nameTeam3 दल 3 नाम
server.config.nameTeam4 दल 4 नाम
server.config.nameTeam5 दल 5 नाम
server.config.t1Dmg दल 1 डैमेज
server.config.t2Dmg दल 2 डैमेज
server.config.t3Dmg दल 3 डैमेज
server.config.t4Dmg दल 4 डैमेज
server.config.t5Dmg दल 5 डैमेज
server.config.selTeam दल चुनो
server.config.allowSpect स्पेक्टेटिंग
server.config.killRewards मारने का पुरस्कार
server.config.headshotOnly केवल हेडशॉट्स
server.config.noSecondary सेकंडरिस ऑफ़
server.config.noStreaks धारियाँ ऑफ़
server.config.canSlide स्लाइडिंग
server.config.wallJP दीवार से कूदने की शक्ति
server.config.autoJump औटो कूदना
server.config.bDrop बुलेट ड्रॉप
server.config.thirdPerson थर्ड पर्सन
server.config.requireCaptcha कैप्चा की आवश्यकता
server.config.nameTags नाम के टैग छुपाए
server.config.kCams किल केम्स
server.config.noReload रीलोड ऑफ
server.config.private प्राइवेट
server.config.krRewards के.आर के पुरस्कार
server.config.forceDefault डिफ़ॉल्ट सेटिंग्स को बलपूर्वक लागू करे
server.config.warmupTime वार्मअप का समय
server.config.gamRounds दौर
server.config.objtvTime उद्देश्य का समय
server.config.forceSpawn औटो रेस्पॉन समय
server.config.scoreLimit स्कोर सीमा
server.config.keepTScore दल का स्कोर रखे
server.config.forceC Force Mode Classes
server.config.logTim समय का पता (दौड़,पारकोर)
server.config.hitBoxPad हिटबॉक्स पैडिंग
server.config.hpRegen स्वास्थ्य पुनर्जन्म
server.config.disableB सीमाओं को निष्क्रिय करें
server.config.throwMel फेंकने योग्य चाकु
server.config.noCosm काज़्मेटिक
server.config.tstCmp जाँच मोड
server.config.limitClasses क्लास लिमिट करें
server.config.tmSize टीम साइज़
social.login लॉगिन/रजिस्टर
social.login.failed लॉगिन असफल हुआ
social.login.buy खरीदने के लिए लॉगिन करे
social.login.sell बेचने के लिए लॉगिन करे
social.login.gift भेट देने के लिए लॉगिन करे
social.login.proccessing लॉगिन हो रहा है...
social.since अकाउंट निर्माण तिथि -
social.user.maps उपयोगकर्ता के खेल
social.user.mods उपयोगकर्ता के मोड्स
social.maps.none कोई खेल नहीं मिले...
social.mods.none कोई मोड्स नहीं मिले...
social.market.price.low सबसे कम मूल्य
social.market.price.list बेचने की कीमत
social.market.searching खोजा जा रहा है...
social.market.listed बेचने के लये रखी {0} पहले
social.market.list आइटम बेचने के लिए रखे
social.market.unlist बेचने की सूची से हटाए
social.market.unlisting आइटम को बेचने की सूची से हटा रहे है...
social.market.posting आइटम्स पोस्ट हो रही है...
social.market.req पद {0} आवश्यक है
social.market.cant खरीदने मई समर्थ नहीं
social.market.purchase खरीदे
social.market.fee बेचने के लिए कोई शुल्क नहीं है
social.market.failed असफल हुए. फिर से प्रयास करे
social.market.head टोपी की आइटम
social.market.back बैक की आइटम
social.market.waist कमर की आइटम
social.market.face चेहरे की आइटम
social.market.shoe जूते
social.market.pet पेट
social.market.melee चाकू
social.market.spray स्प्रे
social.market.dye डाई
social.market.none कोई आइटम नहीं खोजी गयी
social.market.login लॉग इन होना आवश्यक है.
social.market.grace {0} में तैयार
social.market.sort छन्न करे/छांटे
social.market.filter ढूंढे
social.market.sort2 छांटे
social.market.price.high अधिकतम कीमत
social.trading.submitting ऑफ़र सबमिट हो रहा है ...
social.market.gift.amount में भेट भेजने की राशि
social.market.fee.gift निः शुल्क
social.market.gift KR भेट करे
social.market.gift.msg आपका कोई सन्देश?
respawn.auto {0} सेकंड के बाद, आप रेस्पॉन करेंगे
custom.host.ranked आप एक रैंक मैच के दौरान, कोई और खेल होस्ट नहीं कर सकते
funds.insufficient ना-काफी पूंजी
funds.out ऐसा लगता है की आपके पास कम KR है
custom.passcode.long पासवर्ड बहुत लंबा है
custom.passcode.input कृपया सर्वर पासवर्ड डाले
custom.tryagain कृपया पुन: प्रयास करें।
purchase.kr.success {0} KR आपके अकाउंट में डाल दिए गए है
limited.purchase.ask क्या आप [{0}] को {1} KR में खरीदना चाहेंगे?
limited.purchase.success आपके इन्वेंटरी में {0} डाल दिया गया है
pretend.exit एग्जिट करने के लिए, एक बार फिर से {0} दबाये
settings.loaded सेटिंग्स लोड  हो गयी
settings.loaded.none कोई सेटिंग्स नहीं मिले
account.login.required लॉग इन करना होगा
account.banned.hack इस अकाउंट को हैकिंग के लिए बैन किया गया है।
account.banned.appeal यदि आप अपने बैन की अपील करना चाहते हैं, तो संपर्क करें:
account.delete आप अपने अकाउंट को हटाने का अनुरोध कर सकते है। यदि आपके द्वारा किया गया अनुरोध 14 दिनों में रद्द नहीं किया जाता है, तो आपके सारे डेटा को नष्ट कर दिया जायेगा।
account.delete.request क्या आप वाकई अपने अकाउंट और उससे जुड़े सारे डाटा को नष्ट करवाना चाहते है?
account.delete.cancel क्या आप वास्तव में अपना अकाउंट हटाने के अनुरोध को रद्द करना चाहते हैं?
spin.free.available मुफ्त KR उपलब्ध है!
spin.free.claim हर घंटे, मैच खत्म होने के बाद, इस स्पिन को स्पिन किया जा सकता है।
spin.free.disable यह सन्देश आपके सेटिंग्स में जाकर हटाया जा सकता है।
mod.reset.0 रेस्पॉन होने के बाद इसका असर होगा
mod.reset.1 मैप के चीज़ो को बदला नहीं जा सकता
windows.profile.picture प्रोफाइल पिक्चर बदले
stats.assists असिस्ट
stats.nukes न्यूक
stats.melee चाकू किल
stats.fistkills हाथो द्वारा किये गए किल
stats.thrownkills फेंके गए चाक़ू द्वारा किल
stats.headshots हेड शाट
stats.wallbangs वॉल बैंग
stats.sprays स्प्रै काउन्ट
windows.host.default.settings मैप के सेटिंग्स को सक्षम करने से प्लेयर्स के डाटा (हाइ स्कॉर्ज़) को सेव किया जा सकता है
windows.host.default.commands *होस्ट कमांड (/ noclip, /god, /smite, आदि) इस मोड में काम नहीं करेंगे
windows.host.rewards यदि आप KR रिवार्ड्स को सक्षम करते है, तो सारे खिलाड़ियों को डिफ़ॉल्ट सेटिंग्स पर खेलना पड़ेगा और वे आपके मैप के खाते से KR भी जीत सकते है:
windows.store.wheels.sub इस स्पिन के साथ आप  एक नया स्किन पा सकते है, स्पिन करे और अपनी किस्मत आज़माये!
windows.store.currency * अमेरिकी डॉलर में आमदनी आपके देश की मुद्रा में बदल जाएगी और उसका रेट आपकी भुगतान सेवा या बैंक पर निर्भर करेगी।
windows.store.creator अपने पसंदीदा क्रीएटर की सहायता के लिए उनके क्रीएटर कोड का उपयोग करें
windows.chall.customs चुनौतियों को कस्टम खेलो में पूरा नहीं किया जा सकता
twitch.link क्या आप वास्तव में अपना Twitch अकाउंट लिंक करना चाहते है?
twitch.unlink क्या आप वास्तव में अपना Twitch अकाउंट अनलिंक करना चाहते है?
windows.clan.leave क्या आप वास्तव में क्लैन को छोड़ना चाहते हैं?
windows.clan.delete क्या आप वास्तव में क्लैन और उसके डेटा को मिटाना चाहते है?
windows.clan.kick क्या आप वास्तव में {0} को क्लैन से निकालना चाहते हैं?
windows.store.premium.purchase क्या आप वास्तव में {0} दिनों के लिए Premium खरीदने वाले हैं?
windows.playerlist.ban क्या आप वास्तव में {0} को खेल से बैन करने वाले है?
windows.playerlist.flag क्या आप वास्तव में {0} को हैकरटैग चाहते वाले हैं?
windows.playerlist.unflag क्या आप वास्तव में {0} का हैकरटैग हटाने चाहते हैं?
windows.playerlist.punish क्या आप वास्तव में {0} पर कार्रवाई करना चाहते हैं?
item.view.open मॉडल व्यूअर खोलें
item.view.more अधिक जानकारी
item.sell.cant आइटम बेचा नहीं जा सकता
item.sell.quick क्या आप वास्तव में {0} की शीघ्र बिक्री करना चाहते है?
social.profile.stats संपूर्ण आँकड़े
social.profile.class हथियार के आँकड़े
social.profile.sales बिक्री
social.profile.sales.market मार्केट में बिक्री
social.profile.skins स्किन्स [{0}]
social.profile.skins.none उपयोगकर्ता द्वारा कोइ भी स्किन नही
social.profile.games खेल [{0}]
social.profile.games.none उपयोगकर्ता द्वारा कोइ भी खेल नही
social.profile.assets एसेट्स [{0}]
social.profile.assets.none उपयोगकर्ता द्वारा कोइ भी एसेट्स नही
social.profile.mods मॉड्स [{0}]
social.profile.mods.none उपयोगकर्ता द्वारा कोइ भी मॉड्स नही
social.search खोज के परिणाम
social.market.stats.rare सबसे दुर्लभ वस्तु
social.market.stats.circulation सर्कुलेशन मे
social.market.stats.units दुनिया में कुल युनिट्स
social.trades.in प्राप्त
social.trades.in.head प्राप्त ट्रेड्स ({0})
social.trades.out भेजे गए
social.trades.out.head भेजी गयी ट्रेड्स ({0})
social.trades.history इतिहास
social.trades.history.head व्यापार का इतिहास - ({0})
social.trades.decline सभी ट्रेड्स को खारिज करे
social.trades.message कृपया सेटिंग से व्यापार संदेश सक्षम करें
social.trades.warning आपको इस व्यापार से कोइ वस्तु नही मिलेगा!
social.trades.accept क्या आप वास्तव में इस ट्रेड को स्वीकार करना चाहते हैं?
stats.airdrops KR पैकेज
settings.quality.flapAnim स्नाइपर फ्लैप एनीमेशन
`;
